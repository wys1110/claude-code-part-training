...
- Make `https://wys1110.github.io/claude-code-part-training/` the canonical URL for the 73-slide Neon Terminal workshop.
- Keep `https://wys1110.github.io/claude-code-part-training/solution-pe-portfolio-workshop/` as a backward-compatible alias that serves the same deck bytes.
- Exclude the previous 29-slide root deck assets from the Pages artifact while preserving their source files in the repository.
...
- Verify both public URLs with HTTP status, identical SHA-256, page title, slide count, and representative navigation before reporting deployment complete.
