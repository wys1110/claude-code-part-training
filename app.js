...
if(slides.length!==44||total!==120)throw new Error(`Invalid deck: ${slides.length} slides / ${total} minutes`);
...
function markup(s,print=false){return `${s.kicker?`<p class="kicker">${esc(s.kicker)}</p>`:`<p class="kicker">${esc(s.section)} · ${s.minutes} min</p>`}<h1>${esc(s.title)}</h1><div class="content">${content(s)}</div>${print?`<div class="print-meta">${s.id} / 44 · ${s.minutes} min</div>`:''}`;}
function parseHash(){const m=location.hash.match(/slide=(\d+)/);index=Math.min(43,Math.max(0,(m?Number(m[1]):1)-1));}
function closeLayers(){el.notes.hidden=true;el.overview.hidden=true;el.help.hidden=true;}
function render(updateHash=true){const s=slides[index];el.slide.innerHTML=markup(s);el.counter.textContent=`${s.id} / 44`;el.section.textContent=`${s.section} · ${s.minutes} min`;el.progress.style.width=`${((index+1)/44)*100}%`;el.notesBody.innerHTML=`<h2>${s.id}. ${esc(s.title)}</h2><p><strong>예상 시간:</strong> ${s.minutes}분</p><h3>말할 핵심</h3><p>${esc(s.notes)}</p><h3>참가자 행동</h3><p>${esc(s.action)}</p><h3>전환 문장</h3><p>${esc(s.transition)}</p>`;if(updateHash)history.replaceState(null,'',`#slide=${s.id}`);document.title=`${s.id}. ${s.title} · Claude Code Training`;}
function go(i){index=Math.min(43,Math.max(0,i));closeLayers();render();}
...
document.addEventListener('keydown',e=>{if(e.target.matches('input,textarea'))return;if(['ArrowRight','PageDown',' '].includes(e.key)){e.preventDefault();next();}else if(['ArrowLeft','PageUp'].includes(e.key)){e.preventDefault();prev();}else if(e.key==='Home')go(0);else if(e.key==='End')go(43);else if(e.key.toLowerCase()==='o')toggle(el.overview);else if(e.key.toLowerCase()==='n')toggle(el.notes);else if(e.key.toLowerCase()==='f')fullscreen();else if(e.key.toLowerCase()==='p')window.print();else if(e.key==='?')toggle(el.help);else if(e.key==='Escape')closeLayers();});
